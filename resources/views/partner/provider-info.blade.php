...
@section('content')
    <div class="col-md-12">
        ...
        <ol class="mb-4 p-0 ps-3">
            <li class="numbered">Download this <span><a href="{{ asset('assets/sample/zip_list_template.csv') }}"
                        class="text-decoration-underline">zip_list_template.csv</a></span> </li>
            <li class="numbered">
                <div class="tableTerms p-0 border-0">
                    <div class="accordion accordion-flush" id="accordionExample">
                        <div class="accordion-item">
                            <h2 class="accordion-header">
                                <button class="accordion-button provider-accordion-button collapsed m-0 p-0  text-danger"
                                    type="button" data-bs-toggle="collapse" data-bs-target="#collapseOne"
                                    aria-expanded="false" aria-controls="collapseOne">
                                    <span class="text-dark button-text">Add your availability to the zip list
                                        template.</span>&nbsp;Important info for the CSV file <span class="ms-1"><i
                            ...
                            <div id="collapseOne" class="accordion-collapse collapse">
                                <div class="accordion-body">
                                    <ul id="billing">
                                        <li class="billing">
                                            File name can be changed. Column names and order <strong>should not</strong> be
                                            changed.
                                        </li>
                                        <li class="billing">
                                            Rows must be unique based on the combination of ZIP, Type and CustomerType.
                                            <ul>
                                                <li class="provider">A file <strong>may</strong> contain multiple rows with
                                                    the same ZIP if each row has a unique Type and/or CustomerType. For
                                            ...
                                            <ul>
                                                <li class="provider">A file <strong>may not</strong> contain multiple rows
                                                    with the same ZIP if the Type and/or CustomerType is not unique. For
                                                    example the following <strong>is not</strong> acceptable.
                                                    <ul>
                                                        <li class="data">“00544, 10, Fiber, .99, Residential”</li>
                                        ...
                                        <li class="billing">
                                            ...
                                            <ul>
                                                <li class="provider">Zips with preceding zeros can be added with or without
                                                    the zeros. For example, 00544 zip code can be entered as 544.</li>
                                            </ul>
                                        </li>
                                        <li class="billing">
                                            <strong>Speed:&nbsp;</strong> Represents the download speed maximum available in
                                            that zip code. This number should be in Mbps when uploaded.
                                        </li>
                                        <li class="billing">
                                            <strong>Type:&nbsp;</strong> The technology type associated with service in the
                                            zip code. Please use the following options: 5G Home, Cable, DSL, Fiber, Fixed
                                            Wireless, LTE Home, Mobile, Other Copper Wireline, Satellite
                                        </li>
                                        <li class="billing"> <strong>Coverage:&nbsp;</strong> Percentage of the zip code
                                            area covered by the service. Use the decimal representation. For example, 100%
                                            would be 1 and 74% would be 0.74.</li>
                                        <li class="billing"> <strong>CustomerType:&nbsp;</strong> Business or Residential
                                        </li>
            ...
            <div>
                <ul>
                    @foreach ($errors->all() as $error)
                        <li class="text-danger">One or more fields are missing from your file. Please check if your file
                            looks like our template and try again. </li>
                    @endforeach
                </ul>
        ...
        <form id="csvForm" action="{{ route('upload.csv', [], false) }}" method="POST" enctype="multipart/form-data">
            ...
            <div>
                <button type="button" class="btn btn-primary text-center text-white px-3 mb-4 mt-3" id="openModal">Upload
            ...
            <div class="modal-dialog modal-dialog-scrollable modal-lg">
                <div class="modal-content bg-popup ">
                    ...
                    <div class="modal-footer d-flex flex-row justify-content-end bg-popup">
                        <div>
                            <span><strong>Are you sure your file contains all the required data in the correct
                                    format?</strong> <span class="ms-2 me-2" id="fileName"></span></span>
                        </div>
        ...
        <div class="top-row w-100 mt-4">
            <div class="row ">
                <div class="col-md-11">
                    <form action="{{ route('partner.provider-availability-data', [], false) }}" method="GET"
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-1">
                                <div class="input-group mt-4">
                                    <button class="btn button-clearlink text-primary fw-bold"
            ...
            <div class="row">
                <form action="{{ route('partner.provider-availability-data', [], false) }}" method="GET"
                    ...
                    <div class="col-md-1">
                        <div class="input-group">
                            <button class="btn text-primary text-decoration-underline fw-bold p-0 pt-2 mb-1"
                        ...
                        <tr>
                            ...
                            <td>{{ $data->status }}
                                @if (!empty($data->message))
                                    ...
                                    <i class="fa fa-info-circle info-icon" data-message="{{ $data->message }}"
                                        style="cursor: pointer; margin-left: 5px;">
                                    </i>
                                @endif
                            </td>
                            <td>
                                <a class="btn btn-primary btn-sm ms-3 download-button" target="_blank"
                                    href="/download/{{ $data->url }}">Download</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
                        ...
                        @if ($aoa_data->lastPage() > 1)
                            <ul class="pagination">
                                <li class="{{ $aoa_data->currentPage() == 1 ? 'disabled' : '' }}">
                                    <a href="{{ $aoa_data->appends(request()->query())->url(1) }}"
                                        class="page-link">{{ __('First') }}</a>
                                </li>
                                @for ($i = 1; $i <= $aoa_data->lastPage(); $i++)
                                    <li class="{{ $aoa_data->currentPage() == $i ? 'active' : '' }}">
                                        <a href="{{ $aoa_data->appends(request()->query())->url($i) }}"
                                            class="page-link{{ $aoa_data->currentPage() == $i ? ' active' : '' }}">{{ $i }}</a>
                                    </li>
                                @endfor
                                <li class="{{ $aoa_data->currentPage() == $aoa_data->lastPage() ? 'disabled' : '' }}">
                                    <a href="{{ $aoa_data->appends(request()->query())->url($aoa_data->lastPage()) }}"
