                        ...
                        <thead class="border-bottom  shadow">
                            <tr>
                                ...
                                @foreach ($plans as $plan)
                                    <div class="modal fade" id="addonConfirmationModal{{ $plan['plan_id'] }}" tabindex="-1"
                                        aria-labelledby="exampleModalLabel" aria-hidden="true">
                                        <div class="modal-dialog">
                                            <div class="modal-content bg-popup">
                                                <div class="modal-header">
                                                    <p class="modal-title fw-bold me-3 mt-3" id="exampleModalLabel">If you
                                                        have any add-ons, you may lose their benefits. Do you still want to
                                                        upgrade?</p>
                                                ...
                                                <div class="modal-footer">
                                                    <a class="btn btn-primary btn-sm popup-element"
                                                        href="change-plan/{{ $plan['plan_id'] }}" id="save">Proceed</a>
                                    ...
                                    <th class=" align-middle position-relative">
                                        ...
                                        <div>
                                            @if (!$plan['is_current_plan'] && $plan['is_enterprise_plan'] && $plan['plan_code'] == 'enterprise')
                                                <h5 class="mb-2"><strong><span class="text-primary">Custom</span><span>
                                                            Enterprise</span></strong></h5>
                                                @if ($availability_data === null || $company_info === null)
                                                    <a data-bs-toggle="modal" data-bs-target="#showAlertModal"
                                                        id="save" class="btn btn-primary">Contact Us</a>
                                                @else
                                            ...
                                            @elseif($plan['is_current_plan'])
                                                ...
                                                @if ($subscription->addon)
                                                    ...
                                                    <h5 class="text-dark mb-2">
                                                        <strong>${{ number_format($plan['price'], 2) }} +
                                                            {{ number_format($addon->addon_price, 2) }}</strong>
                                                    </h5>
                                                @else
                                                    <h5 class="text-dark mb-2">
                                                ...
                                                @endif
                                                <span
                                                    class="position-absolute top-0 start-50 rounded-1 border border-2 fs-6 translate-middle badge text-primary bg-white">Current
                                                    Plan</span>
                                                @if (!empty($plan['addon']))
                                                    <p class="mt-1 fw-normal p-1"><small>You have also Subscribed to:
                                                            <span>{{ ucwords($plan['plan_name']) }}</span> ADD-ON for the
                                                            current month</small></p>
                                                @elseif(empty($plan['addon']) && $plan_hasaddon)
                                                    @if ($number_of_addons > 1)
                                                        <button type="button" class="btn btn-primary mt-2 mb-2"
                                                            data-bs-toggle="modal" data-bs-target="#selectAddon">
                                                            Monthly Click Add-On
                                                        </button>
                                                    @else
                                                        <a href="addon-plan/{{ $plan['plan_id'] }}" id="save"
                                                            class="btn btn-primary mt-2 mb-2">Monthly Click Add-On</a>
                                                    @endif
                                                @endif
                                                <p class="mt-2 mb-2"><small>Next Renewal Date:
                                                        {{ Carbon\Carbon::parse($plan['next_billing_at'])->format('d-M-Y') }}</small>
                                                </p>
                                            ...
                                            @else
                                                ...
                                                @if ($plan['is_upgrade_possible'])
                                                    @if ($subscription->addon)
                                                        <a style="cursor: pointer;" data-bs-toggle="modal"
                                                            data-bs-target="#addonConfirmationModal{{ $plan['plan_id'] }}"
                                                            class="btn btn-primary">Upgrade</a>
                                                    @else
                                                        <a href="change-plan/{{ $plan['plan_id'] }}" id="save"
                                                            class="btn btn-primary">Upgrade </a>
                                                    @endif
                                                @endif
                                                ...
                                                @if ($plan_subscribed === null)
                                                    @if ($availability_data === null || $company_info === null)
                                                        <a style="cursor: pointer;" data-bs-toggle="modal"
                                                            data-bs-target="#showAlertModal"
                                                            class="btn btn-primary">Subscribe</a>
                                                    @else
                                                        <a href="subscribe-plan/{{ $plan['plan_id'] }}" id="save"
                                                            class="btn btn-primary text-white">Subscribe</a>
                                                    @endif
